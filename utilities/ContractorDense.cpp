/*
 * ContractorDense.cpp, part of Hadrons (https://github.com/aportelli/Hadrons)
 *
...
namespace ContractorDense
{
    const std::size_t nSc = Ns*Nc;
    ...
    public:
        GRID_SERIALIZABLE_CLASS_MEMBERS(CorrelatorResult,
                                        std::vector<ContractorDense::A2AMatrixPar>,  a2aMatrix,
...
struct ContractorDensePar
{
    ContractorDense::GlobalPar                  global;
    std::vector<ContractorDense::A2AMatrixPar>  a2aMatrix;
    std::vector<ContractorDense::ProductPar>    product;
};
...
void saveCorrelator(const ContractorDense::CorrelatorResult &result, const std::string dir,
...
int main(int argc, char* argv[])
{
    ...
    std::map<std::string, A2AModeSpace>              modeSpace;
    ...
    for (auto &p: par.a2aMatrix)
    {
        ...
        space.nLow = p.nLow;
        space.nSc  = ContractorDense::nSc;
        space.nt   = par.global.nt;
        space.nHit = par.global.nHit;
        a2aMat.emplace(p.name, EigenDiskVector<ComplexD>(dirName, par.global.nt, p.cacheSize));
        modeSpace.emplace(p.name, space);
        std::cout << "A2A matrix '" << p.name << "': dense " << space.denseSize()
                  << ", expanded " << space.expandedSize() << std::endl;
    }
         ...
         traj < par.global.trajCounter.end; traj += par.global.trajCounter.step)
    {
        ...
        for (auto &p: par.product)
        {
            ...
            A2AMatrix<ComplexD>                    prod;
            TimerArray                             tAr;
            double                                 fusec, busec, flops, bytes;
            ContractorDense::CorrelatorResult      result;
            ...
            if (term.size() != 2)
            {
                HADRONS_ERROR(Size, "ContractorDense only supports 2 terms, got "
                            + std::to_string(term.size())
                            + " (use Contractor for the expanded case)");
            }
            ...
            for (auto &s: term)
            {
                if (modeSpace.find(s) == modeSpace.end())
                {
                    HADRONS_ERROR(Definition, "no a2aMatrix named '" + s + "'");
                }
            }

            const A2AModeSpace &spaceI = modeSpace.at(term[0]);
            const A2AModeSpace &spaceJ = modeSpace.at(term[1]);
            ...
            std::cout << timeSeq.size()*translations.size()*par.global.nt << " tr(A*B)"
                    << std::endl;
            ...
            for (unsigned int i = 0; i < timeSeq.size(); ++i)
            {
                ...
                for (auto &dt: translations)
                {
                    ...
                    for (unsigned int tLast = 0; tLast < par.global.nt; ++tLast)
                    {
                        ...
                        A2AContraction::accTrMulDense(result.correlator[TIME_MOD(tLast - dt)],
                                                      prod, TIME_MOD(t[0] + dt),
                                                      lastTerm[tLast], tLast,
                                                      spaceI, spaceJ);
                        tAr.stopTimer("tr(A*B)");
                        flops += A2AContraction::accTrMulDenseFlops(spaceI, spaceJ);
