 ...
 * Test_mmf_blas.cpp
 *
 * Hadrons application: runs A2AMomMesonFieldBLAS (momentum-factored BLAS
 * Phase 2 path) on random A2A vectors and writes HDF5 output to
 * mmf_blas_out.<traj>/.
 *
 * Run with the same --seed and --grid as Test_mf_gpu and Test_mmf_kernel to
 * get identical random vectors across all three, then diff the HDF5 outputs:
 *
 *   h5diff mmf_blas_out.0/Gamma5_0_0_0.h5 nmf_gpu_out.0/Gamma5_0_0_0.h5 \
 *          /Gamma5_0_0_0 /Gamma5_0_0_0
 *
 * Usage:
 *   mpirun -n <N> ./Test_mmf_blas --grid 4.4.4.8 --mpi 1.1.1.1 \
...
int main(int argc, char *argv[])
{
    ...
    // ------------------------------------------------------------------
    // Global parameters - must match Test_mf_gpu.cpp and Test_mmf_kernel.cpp
    ...
    // ------------------------------------------------------------------
    // CLI arguments - must match Test_mf_gpu.cpp and Test_mmf_kernel.cpp
    ...
    // and Test_mmf_kernel.cpp exactly.
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // A2AMomMesonFieldBLAS - custom kernel Phase 1, gemmBatched Phase 2
    // ------------------------------------------------------------------
    MContraction::A2AMomMesonFieldBLASPar mmfbPar;
    mmfbPar.block  = block;
    mmfbPar.left   = "left";
    mmfbPar.right  = "right";
    mmfbPar.output = "mmf_blas_out";
    mmfbPar.gammas = gammas;
    mmfbPar.mom    = momenta;

    application.createModule<MContraction::A2AMomMesonFieldBLAS>("mmf_blas", mmfbPar);
