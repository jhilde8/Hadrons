/*
 * Test_cmof_mt_cpu.cpp
 *
 * Hadrons application: runs A2AChromoMagneticOperatorFieldMT (CPU reference)
 * on random A2A vectors with a random gauge field and writes HDF5 output to
 * cmof_mt_out.<traj>/.
 *
 * Run alongside Test_cmof_gpu.cpp (same seed -> identical random vectors and
 * gauge configuration) then diff the HDF5 outputs to validate the GPU module.
 *
 * Usage:
 *   mpirun -n 1 ./Test_cmof_mt_cpu --grid 4.4.4.8 --mpi 1.1.1.1 --seed "1 2 3 4"
 *
 * The --seed argument must match Test_cmof_gpu.cpp to get identical random
...
int main(int argc, char *argv[])
{
    ...
    // ------------------------------------------------------------------
    // Global parameters - must match Test_cmof_gpu.cpp exactly so that
    ...
    // ------------------------------------------------------------------
    // Parse optional CLI arguments (must match Test_cmof_gpu.cpp values
    ...
    // Name must match Test_cmof_gpu.cpp exactly.
    // ------------------------------------------------------------------
    application.createModule<MGauge::Random>("gauge");

    // ------------------------------------------------------------------
    // Random A2A vectors
    // Names and sizes must match Test_cmof_gpu.cpp exactly.
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // A2AChromoMagneticOperatorFieldMT - CPU reference
    // ------------------------------------------------------------------
    MContraction::A2AChromoMagneticOperatorFieldMTPar cmofPar;
    cmofPar.cacheBlock = cacheBlock;
    cmofPar.parities   = "0 1";
    cmofPar.left       = "left";
    cmofPar.right      = "right";
    cmofPar.gauge      = "gauge";
    cmofPar.output     = "cmof_mt_out";
    cmofPar.ifOrthogs  = "0 1";

    application.createModule<MContraction::A2AChromoMagneticOperatorFieldMT>("cmof_mt", cmofPar);
