 ...
 * Test_amf_gpu.cpp
 *
 * Hadrons application: runs A2AAllMesonField (spin-split BLAS, single GEMM
 * per momentum block) on random A2A vectors and writes HDF5 output to
 * amf_gpu_out.<traj>/.
 *
 * Run alongside Test_fmf_cpu.cpp (same seed -> identical random vectors) then
 * diff the HDF5 outputs to validate the gamma trace against the CPU reference.
 *
 * Usage:
 *   mpirun -n 1 ./Test_amf_gpu --grid 4.4.4.8 --mpi 1.1.1.1 --seed "1 2 3 4"
 *
 * The --seed argument must match Test_fmf_cpu.cpp to get identical random
 * vectors.  The runId and trajectory counter are hard-coded to match.
 *
 * Output files: amf_gpu_out.0/<gamma>_<mom>.h5
 *   e.g. amf_gpu_out.0/Gamma5_0_0_0.h5
 *
 * Compare against Test_fmf_cpu output with h5diff, e.g.:
 *   h5diff amf_gpu_out.0/Gamma5_0_0_0.h5 fmf_cpu_out.0/Gamma5_0_0_0.h5 \
...
int main(int argc, char *argv[])
{
    ...
    // ------------------------------------------------------------------
    // A2AAllMesonField - spin-split BLAS path, single GEMM per mom block.
    // ------------------------------------------------------------------
    MContraction::A2AAllMesonFieldPar amfPar;
    amfPar.block  = block;
    amfPar.left   = "left";
    amfPar.right  = "right";
